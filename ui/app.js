      ...
      <div class="card" style="margin-top:20px">
        ...
        <table>
          ...
          <tbody>${targets.map(t => {
            ...
            const pk = t.pocket ? pocketMap[t.pocket] : null;
            return `
            <tr>
              <td>
              ...
              <td><span style="color:${t.status === 'completed' ? 'var(--green)' : t.status === 'paused' ? 'var(--yellow)' : 'var(--accent)'}">${h(t.status || 'active')}</span></td>
              <td class="actions">
...
async function showSavingsTargetForm(id) {
  ...
  try {
    const { data: pockets } = await API.get('/api/pockets');
    pocketOptions = pockets.filter(p => !p.isArchived).map(p => `<option value="${p.id}" ${target.pocket === p.id ? 'selected' : ''}>${h(p.icon || '💳')} ${h(p.name)}</option>`).join('');
  } catch (e) { /* ignore */ }
...
async function saveSavingsTarget(e, id) {
  ...
  const body = {
    ...
    pocket: f.pocket.value || undefined,
    icon: f.icon.value || undefined,
    color: f.color.value || undefined,
    notes: f.notes.value || undefined,
  };
          ...
          <tbody>${recurrings.map(r => `
            <tr>
              <td>
                ...
                ${r.category ? `<br><small style="color:var(--text2)">${h(catMap[r.category] || '-')}</small>` : ''}
                ${r.pocket ? `<br><small style="color:var(--text2)">${h(pocketMap[r.pocket]?.icon || '💳')} ${h(pocketMap[r.pocket]?.name || '')}</small>` : ''}
              </td>
...
async function showRecurringForm(id) {
  ...
  try {
    ...
    catOptions = categories.map(c => `<option value="${c.id}" ${rec.category === c.id ? 'selected' : ''}>${h(c.name)}</option>`).join('');
    pocketOptions = pockets.filter(p => !p.isArchived).map(p => `<option value="${p.id}" ${rec.pocket === p.id ? 'selected' : ''}>${h(p.icon || '💳')} ${h(p.name)}</option>`).join('');
  } catch (e) { /* ignore */ }
